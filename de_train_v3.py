...
class DEEnv(gym.Env):
    def __init__(self, reward_strategy="R1"):
        self.n_ops = 4
    ...
    def reset_(self, fun=None):
        if fun is None:
            print("ERROR, must input Function")
            exit()

        self.fun = fun
        self.lbounds = np.array(self.fun.min_bounds);
        self.ubounds = np.array(self.fun.max_bounds);
        self.dim = len(self.ubounds)
        opti = self.fun.get_optimal_solutions()
        for o in opti:
            sol = np.copy(o.phenome)
        self.best_value = self.fun.objective_function(sol)
        print("Function info: fun= ", self.fun, " with dim = ", self.dim, " with best value= ", self.best_value)
    ...
    def reset(self):
        ...
        self.window = [[np.inf for j in range(self.number_metric)] for i in range(self.window_size)]
