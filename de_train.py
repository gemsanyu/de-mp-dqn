...
class DEEnv(gym.Env):
    def __init__(self, reward_definition="R1"):
        self.n_ops = 4
        ...
        self.observation_space = spaces.Tuple((
            spaces.Box(low=-np.inf, high=np.inf, shape=(99,), dtype=np.float32),
        ...
        # self.FF = 0.5;
        self.CR = 1.0
        self.func_choice = [unimodal.F1, unimodal.F2, unimodal.F5, basic_multimodal.F6, basic_multimodal.F8,
        ...
        self.func_select_index = np.arange(0, len(self.func_select))
        self.fun_index = 0
    ...
    def step(self, action):
        ...
        bprime = mutate(self.population, self.r, F, self.best, self.i)
    ...
    def reset(self):
        self.dim = self.func_select[self.func_select_index[self.fun_index]][1]
        self.fun = self.func_select[self.func_select_index[self.fun_index]][0](self.dim)
        ...
        print("Function info: fun= ", self.func_select[self.func_select_index[self.fun_index]], " with dim = ", self.dim, " with best value= ", self.best_value)
        self.budget = 1e4
        self.max_budget = 1e4
        self.generation = 0
        ...
        self.budget -= self.NP
        # Make changes to X wherever needed using u and use population to pick random solutions
        self.population = np.copy(self.X)
        self.copy_F = np.copy(self.F)
        ...
        if self.fun_index == len(self.func_select):
            self.fun_index = 0
            random.shuffle(self.func_select_index)
