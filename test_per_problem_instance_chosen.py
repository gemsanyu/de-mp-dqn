...
@click.option('--snapshot-arr-str', default=" ", help="Chosen epoch snapshots", type=str)
@click.option('--problem-idx', default=0, help="0-4 id of problem index", type=int)
@click.option('--problem-dim', default=10, help="10,30 problem dec variable dimension", type=int)
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
        ...
        snapshot_arr_str="123",problem_idx=0, problem_dim=10,):
    ...
    snapshot_arr_str = snapshot_arr_str.split(sep=",")
    snapshot_arr = [int(snapshot) for snapshot in snapshot_arr_str]
    print(snapshot_arr)
    max_runs = 25
    dims = [10, 30]
    func_select = [unimodal.F3, basic_multimodal.F9, f16.F16, f18.F18, f23.F23]
    func_names = ["F3","F9","F16","F18","F23"]
    ...
    d = 10
    fun = unimodal.F3(d)
    lbounds = fun.min_bounds; lbounds = np.array(lbounds)
    ubounds = fun.max_bounds; ubounds = np.array(ubounds)
    ...
    result_file_prefix=reward_strategy+"_"+str(problem_idx)+"_"+str(problem_dim)+"_"
    result_file_header="RUN,"+func_names[problem_idx]+"-"+str(problem_dim)
    mean_result_file = open(result_file_prefix+"mean.csv", "w+")
    mean_result_file.write(result_file_header+"\n")

    std_result_file = open(result_file_prefix+"std.csv", "w+")
    std_result_file.write(result_file_header+"\n")

    for epoch in snapshot_arr:
