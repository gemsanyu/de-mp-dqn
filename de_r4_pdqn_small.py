...
@click.option('--title', default="DE-PDDQN", help="Prefix of output files", type=str)
@click.option('--reward-strategy', default="R4", help="Prefix of output files", type=str)
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
        ...
        save_freq, save_dir, save_frames, visualise, action_input_layer, title, reward_strategy="R4"):
    ...
    for i in range(episodes):
        ...
        for j in range(max_steps):
            ...
            # print("Episode-",i,"BUDGET", env.budget, ": ", env.copy_F[env.best]," REWARD ", reward)
            # print("Episode-",i)
            next_act, next_act_param, next_all_action_parameters = agent.act(next_state)
            # print("NEXT", next_act, next_act_param, next_all_action_parameters)
            next_action = pad_action(next_act, next_act_param)
            Q_loss = agent.step(state, (act, all_action_parameters), reward, next_state,
